...
namespace small_object_tracker
{
  ...
  public:
    ...
    ObjectDetector() :  min_y_(-10.0), max_y_(10.0),
			...
			kinect_tilt(0.0),
			pre_x(0.0),pre_y(0.0),
			time_lag(0.03),first_detect_(true)
    {
      ...
      ROS_INFO("Thresholds:");
      printf("distance_threshold: %f, centroid_threshold: %f, r_threshold: %d, g_threshold: %d, b_threshold: %d\n",
	     distance_threshold, centroid_threshold, r_threshold, g_threshold, b_threshold);
  ...
  private:
      ...
      BOOST_FOREACH(const pcl::PointXYZRGB& pt, cloud->points)
	{
	  ...
	  if(!std::isnan(cent_x) && !std::isnan(cent_y) && !std::isnan(cent_z))
	      ...
	      if (-pt.y > min_y_ && -pt.y < max_y_ && pt.x < max_x_ && pt.x > min_x_ && pt.z < max_z_ && pt.z >min_z_)
		{
		  ...
		  //		  		  std::cout<<"[discover] RGB value is"<<(int)pt.r<<" "<<(int)pt.g<<" "<<(int)pt.b <<std::endl;
		  //  if( point_color_decision( (int)pt.r , (int)pt.g, (int)pt.b) ){ // 9/15に取り除く。色認識を除く
      ...
      if(smoothing_mode)
	{
      ...
      if(first_detect_) {pre_x = coordinate_on_table.x;pre_y = coordinate_on_table.y;first_detect_ = false;
	std::cout<<"first detect"<<std::endl;
      }
	...
	*/
    ...
    void position_sending(double x, double y, double z){
      ...
      //x_vel_offsetとy_vel_offsetの計算
      x_vel_offset = ((x - pre_x)/dt)*(time_lag+dt); 
      y_vel_offset = ((y - pre_y)/dt)*(time_lag+dt); 
      pre_x = x;pre_y = y; 

      //速度調整のモデルを入れる
      x += x_vel_offset;
      y += y_vel_offset;
      if(fabs(x_vel_offset) > 0.01)      std::cout<<"x_vel_offset: "<<x_vel_offset<<std::endl;
      if(fabs(y_vel_offset) > 0.01)      std::cout<<"y_vel_offset: "<<y_vel_offset<<std::endl;
	     //      std::cout<<x_vel_offset<<" "<<y_vel_offset<<std::endl;
